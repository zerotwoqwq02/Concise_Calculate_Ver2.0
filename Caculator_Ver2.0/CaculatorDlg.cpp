...
void  database(void) {
	...
	rc = sqlite3_open("history.txt", &db); // 打开或创建数据库文件 history.db
	...
	if (rc) {
		...
		const char* createTableSQL = "CREATE TABLE HISTORY (ID INTEGER PRIMARY KEY AUTOINCREMENT, EQUATION TEXT, RESULT TEXT);";
	...
	CString expression; // 从你的计算器获取表达式
	CString result;     // 从你的计算器获取结果
	...
	if (rc) {
		...
		CString insertSQL;
		insertSQL.Format(_T("INSERT INTO HISTORY (EQUATION, RESULT) VALUES ('%s', '%s');"), expression, result);
		rc = sqlite3_exec(db, CStringA(insertSQL), 0, 0, &zErrMsg);
...
ON_BN_CLICKED(IDC_HISTORY_BUTTON, &CYourDialogClassName::OnBnClickedHistoryButton)

// 声明消息处理函数
afx_msg void OnBnClickedHistoryButton();

// 实现消息处理函数
void CYourDialogClassName::OnBnClickedHistoryButton()
{
	// 在这里显示历史记录，可以使用数据库查询语句检索历史记录并显示在一个对话框中
}
